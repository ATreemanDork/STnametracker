...
async function updateLorebookEntry(character, characterName) {
    ...
    try {
        ...
        if (!worldInfo) {
            debugLog(`WARNING: Could not load lorebook ${lorebookName}. It may not exist yet.`);
            // Lorebook will be created when first entry is added
        }
        
        // Check if entry already exists
        let entryUid = character.lorebookEntryId;
        let existingEntry = worldInfo?.entries?.find(e => e.uid === entryUid);
